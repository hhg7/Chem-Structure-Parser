#!/usr/bin/env perl
# md2pod.pl -- README.md is the documentation; this generates the rest of it.
#
# Adapted from ~/Scripts/stats/md2pod.pl (Stats::LikeR), which is where every
# workaround below was worked out and where the comments explaining them were
# written.  Three things differ here, and only three:
#
#   * the module and distribution names, below;
#   * the POD carries its own "=head1 NAME", taken from the "# ABSTRACT:" line
#     of the module.  dist.ini has no [PodWeaver] to write one, and Makefile.PL
#     reads ABSTRACT_FROM out of that section, so without it the distribution
#     has no abstract at all;
#   * the "__END__" that already separates the code from the POD is kept.
#
# One file comes out of README.md: the POD half of lib/Chem/Structure/Parser.pm
# (everything after the "1;" is replaced, so the module's documentation is never
# edited by hand), plus read.me.pod, which is the same POD on its own for
# inspection.  pod_file_ok checks it before this exits.
#
# Changes used to come out of README.md too, written from a "# Changes" section
# there.  It does not any more: the release notes lived in three places at once
# -- the README, the module's POD and Changes -- and the POD copy in particular
# was a hundred and fifty lines of history in the middle of the reference
# documentation.  Changes is now written by hand, in the form
# CPAN::Changes::Spec asks for, and this script only checks it: changes_file_ok
# still runs at the end, so a release whose Changes does not satisfy the spec
# is still not built.
#
# Run it from the distribution root, or through ./dzil.sh, which runs it first.
...
my $MODULE = 'Chem::Structure::Parser';
my $LIB    = 'lib/Chem/Structure/Parser.pm';
my $DIST   = $MODULE =~ s/::/-/gr;
...
my $lib = file2string($LIB);

# The abstract lives in the "# ABSTRACT:" line, which is where dzil reads it
# from; this puts the same sentence in the "=head1 NAME" that Makefile.PL's
# ABSTRACT_FROM parses, so the two cannot drift apart.  MakeMaker wants the
# exact form "Module::Name - text" on one line.
my ($abstract) = $lib =~ /^\#\s*ABSTRACT:\s*(\S.*?)\s*$/m;
die "md2pod: $LIB has no '# ABSTRACT:' line to build '=head1 NAME' from\n"
	unless defined $abstract;

my @pod = split /\n/, $pod;
unshift @pod, "=encoding utf8\n", "=head1 NAME\n", "$MODULE - $abstract\n";

say "Writing read.me.pod from README.md, and the POD half of $LIB with it";
...
if (!@marks) {
	die "md2pod: no '1;' line in $LIB to append the POD after\n";
}
if (@marks > 1) {
	die "md2pod: $LIB has " . scalar(@marks) . " lines that are "
	  . "just '1;' (at " . join(', ', map { $_ + 1 } @marks) . '); cannot tell '
	  . "which one ends the code\n";
}

# Trim everything after `1;` to prep for new POD insertion.  __END__ goes back
# in: the module has always had one, and nothing after it is compiled at all,
# which is a stronger promise than "the tokenizer skips POD".
...
# Changes is written by hand and is only checked here.  It is checked rather
# than left alone because a Changes file that does not satisfy
# CPAN::Changes::Spec fails the release, and finding that out at `dzil build'
# time is finding it out at the right time.
